...
def save_graph_to_file(sess, graph, graph_file_name):
    output_graph_def = graph_util.convert_variables_to_constants(
        sess, graph.as_graph_def(), ['outputfc', 'weight_new', 'error'])
...
def main():
    ...
    output = tf.reshape(tf.nn.softmax(tf.matmul(input, weight) + bias), [5], name='outputfc') # activation / output
    ...
    #error = tf.metrics.mean_squared_error(true_output, output, name= 'error')
    error = tf.reduce_mean(tf.squared_difference(output, true_output), name='error')
